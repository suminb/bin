...
time ffmpeg -i $(aws --endpoint-url=$endpoint s3 presign "$source") -c:v libx265 -crf 23 -preset fast -c:a aac -b:a 128k -map_metadata 0 -tag:v hvc1 -y -f mp4 -movflags frag_keyframe+empty_moov pipe:1 | aws --endpoint-url=$endpoint s3 cp - $target
